module Agents
  class EventFormattingAgent < Agent
    ...
    description <<-MD
      An Event Formatting Robot allows you to format incoming Events, adding new fields as needed.
      ...
      You may want to send this event to another Robot, for example a Twilio Robot, which expects a `message` key.
      You can use an Event Formatting Robot's `instructions` setting to do this in the following way:
      ...
      Events generated by this possible Event Formatting Robot will look like:
      ...
      By default, the output event will have `robot` and `created_at` fields added as well, reflecting the original Robot type and Event creation time.  You can skip these outputs by setting `skip_robot` and `skip_created_at` to `true`.
    ...
    def validate_options
      errors.add(:base, "instructions, mode, skip_robot, and skip_created_at all need to be present.") unless options['instructions'].present? && options['mode'].present? && options['skip_robot'].present? && options['skip_created_at'].present?
    ...
    def receive(incoming_events)
      incoming_events.each do |event|
        ...
        formatted_event['agent'] = Agent.find(event.agent_id).type.slice!(8..-1) unless opts['skip_robot'].to_s == "true"
