...
module Agents
  class JiraAgent < Agent
    ...
    description <<-MD
      The Jira Robot subscribes to Jira issue updates.
      ...
      The robot does periodic queries and emits the events containing the updated issues in JSON format.
      NOTE: upon the first execution, the robot will fetch everything available by the JQL query. So if it's not desirable, limit the `jql` query by date.
    MD
  ...
  private
    ...
    def request_options
      ropts = {:headers => {"User-Agent" => "Huginn (https://github.com/DataMinerUK/huginn)"}}
